...
(function(global, factory) {
	...
	$.fn.fullpage = function(options) {
		...
		function init() {
			...
			if (activeSlide.length && ($(SECTION_ACTIVE_SEL).index(SECTION_SEL) !== 0 || ($(SECTION_ACTIVE_SEL).index(
					SECTION_SEL) === 0 && activeSlide.index() !== 0))) {
		...
		function styleSlides(section, slides, numSlides) {
			...
			//the active section isn't the first one? Is not the first slide of the first section? Then we load that section/slide by default.
			if (startingSlide.length && ($(SECTION_ACTIVE_SEL).index(SECTION_SEL) !== 0 || ($(SECTION_ACTIVE_SEL).index(
					SECTION_SEL) === 0 && startingSlide.index() !== 0))) {
		...
		function createSlideArrows(section) {
			...
			if (options.controlArrowColor != '#fff') {
				...
				section.find(SLIDES_ARROW_PREV_SEL).css('border-color', 'transparent ' + options.controlArrowColor +
		...
		function afterRenderActions() {
			...
			$.isFunction(options.afterLoad) && options.afterLoad.call(section, section.data('anchor'), (section.index(
				SECTION_SEL) + 1));
		...
		*/
		function touchMoveHandler(event) {
			...
			if (!checkParentForNormalScrollElement(event.target) && isReallyTouch(e)) {
				...
				if (canScroll && !slideMoving) { //if theres any #
					...
					if (activeSection.find(SLIDES_WRAPPER_SEL).length && Math.abs(touchStartX - touchEndX) > (Math.abs(touchStartY -
							touchEndY))) {
		...
		function MouseWheelHandler(e) {
			...
			if (options.autoScrolling && !controlPressed) {
				...
				var isScrollingVertically = (Math.abs(e.wheelDeltaX) < Math.abs(e.wheelDelta)) || (Math.abs(e.deltaX) < Math.abs(
					e.deltaY) || !horizontalDetection);
		...
		function afterSectionLoads(v) {
			...
			$.isFunction(options.afterLoad) && !v.localIsResizing && options.afterLoad.call(v.element, v.anchorLink, (v.sectionIndex +
				1));
		...
		function hashChangeHandler() {
			if (!isScrolling && !options.lockAnchors) {
				...
				if (section.length) {
					...
					if ((section && section !== lastScrolledDestiny) && !isFirstSlideMove || isFirstScrollMove || (!slideMoving &&
							lastScrolledSlide != slide)) {
						scrollPageAndSlide(section, slide);
					}
				}
			}
		...
		function landscapeScroll(slides, destiny) {
			...
			if (options.onSlideLeave) {
				...
				if (!localIsResizing && xMovement !== 'none') {
					if ($.isFunction(options.onSlideLeave)) {
						if (options.onSlideLeave.call(prevSlide, anchorLink, (sectionIndex + 1), prevSlideIndex, xMovement, slideIndex) ===
							false) {
							slideMoving = false;
							return;
						}
			...
			var afterSlideLoads = function() {
				//if the site is not just resizing and readjusting the slides
				if (!localIsResizing) {
					$.isFunction(options.afterSlideLoad) && options.afterSlideLoad.call(destiny, anchorLink, (sectionIndex + 1),
		...
		function addTableClass(element) {
			element.addClass(TABLE).wrapInner('<div class="' + TABLE_CELL + '" style="height:' + getTableHeight(element) +
				'px;" />');
		}
